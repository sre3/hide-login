...
if ( ! class_exists( 'WPS_Hide_Login' ) ) {
	class WPS_Hide_Login {
		...
		public function admin_notices_incompatible() {
			echo '<div class="error notice is-dismissible"><p>' . __( 'Please upgrade to the latest version of WordPress to activate', 'wpserveur-hide-login' ) . ' <strong>' . __( 'WPS Hide Login Lite', 'wpserveur-hide-login' ) . '</strong>.</p></div>';
		}

		public function admin_notices_plugin_conflict() {
			echo '<div class="error notice is-dismissible"><p>' . __( 'WPS Hide Login Lite could not be activated because you already have "Rename WP-Login" active. Please uninstall "Rename WP-Login" to use WPS Hide Login Lite', 'wpserveur-hide-login' ) . '</p></div>';
		}
		...
		public function wpmu_options() {
			...
			$out .= '<h3>' . __( 'WPS Hide Login Lite', 'wpserveur-hide-login' ) . '</h3>';
			...
			$out .= '<p>' . sprintf( __( 'Need help? Try the <a href="%1$s" target="_blank">support forum</a>.', 'wpserveur-hide-login' ), 'http://wordpress.org/support/plugin/wps-hide-login/') . '</p>';
		...
		public function plugin_action_links( $links ) {
			if ( is_network_admin() && is_plugin_active_for_network( WPS_HIDE_LOGIN_BASENAME ) ) {
		...
		public function plugins_loaded() {
			...
			if ( ! is_multisite()
				...
				&& apply_filters( 'wps_hide_login_signup_enable', false ) === false ) {
		...
		public function wp_loaded() {
			...
			if ( $pagenow === 'wp-login.php'
				...
				wp_safe_redirect( home_url( '/404' ) );
				// nrekow: temporary disabled for testing purposes.
